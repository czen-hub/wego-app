...
import { Package, ChevronLeft, MapPin, Clock, Check, X, User } from "lucide-react";
...
export default function CourierJobs() {
  ...
  useEffect(() => {
    const unsub = listenToPendingJobsByType("courier", (rides) => {
      setJobs(rides);
      setLoaded(true);
    });
    return unsub;
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Package pickup and delivery requests</p>
          </div>
        </div>
        ...
        {loaded && visible.length === 0 && accepted.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
            ...
            <div className="space-y-1.5">
              <p className="text-lg font-semibold text-foreground">No courier jobs right now</p>
              <p className="text-sm text-muted-foreground max-w-xs leading-relaxed">
                Open package delivery jobs will appear here. Make sure Courier is enabled in your preferences.
              </p>
            </div>
          </div>
        )}
          ...
          <div key={job.id} className="glass-card border border-border rounded-xl p-4 space-y-4">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-bold text-primary">Package Delivery</p>
            ...
            <div className="flex gap-2">
              ...
              >
                <Check size={15} /> {accepting === job.id ? "Accepting…" : "Accept Job"}
              </button>
            </div>
          </div>
        ))}
        ...
        <div className="flex items-center gap-2 bg-card border border-border rounded-xl px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground">
            Enable Courier in Preferences to receive these jobs on the home screen automatically.
          </p>
        </div>
      </div>
    </div>
